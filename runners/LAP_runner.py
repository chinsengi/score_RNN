...
class LAP():
    ...
    def train(self):
        ...
        LAP_mean = torch.tensor([-1., 1.], device=self.device).reshape([2, 1])
        LAP_std = torch.tensor([0.5]).unsqueeze(0).repeat(2, 1).to(LAP_mean)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for i in range(n_level+1):
                model = self.set_model()
                load(f"./model/LAP/{self.args.run_id}/{self.args.model}_ep{i*(self.args.nepochs//self.args.noise_level)}.pth", model)
                model.set_weight()
            ...
            plt.legend(fontsize=12)
            savefig(path="./image/LAP", filename=self.args.model+"_LAP_sampled")
