...
class LAP:
    ...
    def train(self):
        ...
        LAP_mean = torch.tensor([[0.0, 0.0], [0.0, 0.0]])
        ...
        data = LAPData(LAP_mean, LAP_std, n=10000)
        ...
        for epoch in tqdm(range(nepoch)):
            if epoch % (nepoch // n_level) == 0:
                ...
                save(
                    model,
                    optimizer,
                    f"./model/LAP/{self.args.run_id}",
                    f"{self.args.model}_ep{epoch}.pth",
                )
        ...
        save(
            ...
            optimizer,
            f"./model/LAP/{self.args.run_id}",
            f"{self.args.model}_ep{nepoch}.pth",
        )
        save(
            model,
            optimizer,
            f"./model/LAP/{self.args.run_id}",
            f"{self.args.model}_chkpt{self.args.run_id}.pth",
        )
    ...
    def test(self):
        ...
        if self.args.model == "SR":
            # initial_states = (torch.rand([nsample, hid_dim])*4-2).to(self.device)
            initial_states = (torch.randn([nsample, hid_dim])).to(self.device)
        elif self.args.model == "SO_SC" or self.args.model == "SO_FR":
            initial_states = (torch.rand([nsample, out_dim]) * 4 - 2).to(self.device)
        ...
        with torch.no_grad():
            ...
            model = self.set_model()
            load(
                f"./model/LAP/{self.args.run_id}/{self.args.model}_chkpt{self.args.run_id}.pth",
                model,
            )
            model.set_weight()
            model.dt = 1e-4
            ...
            axes[0].hist2d(
                true_samples[:, 0],
                true_samples[:, 1],
                bins=[np.arange(-6, 6, 0.1), np.arange(-6, 6, 0.1)],
                label="true samples",
            )
            ...
            savefig(
                path="./image/LAP",
                filename=self.args.model + f"{self.args.run_id}_LAP_true_v_gen_density",
                format="png",
            )
            ...
            true_kurts = [
                kurtosis(true_samples[:, 0]),
                kurtosis(true_samples[:, 1]),
            ]
            ...
            plt.ylabel("Kurtosis")
            plt.legend(
                loc="upper center", bbox_to_anchor=(0.5, 1.15), ncol=2, fontsize=16
            )
            ...
            savefig(
                path="./image/LAP",
                filename=self.args.model + f"{self.args.run_id}_LAP_true_v_gen_kurt",
                format="png",
            )
            ...
            for i in range(len(traj)):
                wass_dist_first.append(
                    wasserstein_distance(traj[i, :, 0], true_samples[:, 0])
                )
                wass_dist_sec.append(
                    wasserstein_distance(traj[i, :, 1], true_samples[:, 1])
                )
            # save arrays to file
            savenpy(
                "./image/LAP/",
                f"{self.args.run_id}_{self.args.model}_wass_dist_first",
                np.array(wass_dist_first),
            )
            savenpy(
                "./image/LAP",
                f"{self.args.run_id}_{self.args.model}_wass_dist_sec",
                np.array(wass_dist_sec),
            )
