...
class WhiteConnectomeData:
    def __init__(self, cnctmpath, device = 'cuda'):
        ...
        self.__chemdf = pd.read_csv(join(basepath,'chem.csv'), skipinitialspace = True, quotechar = '"').drop('Unnamed: 0', axis = 1)
        self.__gjsym = pd.read_csv(join(basepath,'elec_symmetric.csv'), skipinitialspace = True, quotechar = '"').drop('Unnamed: 0', axis = 1)
        self.__gjassym = pd.read_csv(join(basepath,'elec_asymmetric.csv'), skipinitialspace = True, quotechar = '"').drop('Unnamed: 0', axis = 1)
        self.__spec_neurondf = pd.read_csv(join(basepath , 'herm_specific_cells.csv'), skipinitialspace = True, quotechar = '"')
        ...
        self.__shared_neurondf = pd.read_csv(join(basepath, 'sex_shared_cells.csv'), skipinitialspace = True, quotechar = '"')           
        self.__shared_neurondf.index = list(self.__shared_neurondf['name'])       
        self.__shared_neurondf.drop('name', axis = 1)
        self.white_connectomedf = pd.read_csv(join(basepath, 'white_neuron_connect.csv'), skipinitialspace = True)
...
if __name__ == "__main__":
    c = WhiteConnectomeData('./data/worm_cnct')
    c.save_connectome_as_pickle('./data/white_cnct.pkl')